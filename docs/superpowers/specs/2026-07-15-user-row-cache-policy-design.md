...
Stop follower and following screens from issuing a forced `get_user` request for every rendered cached row. Continue showing cached user data immediately, use the existing shared 30-minute user-cache lifespan for ordinary row loading, and keep profile opening as the explicit forced-refresh boundary.
