...
// MARK: - HproseService
actor HproseInstance: ObservableObject {
    // MARK: - Properties
    static let shared = HproseInstance()
    static var baseUrl: String = ""
    static var appId: String = Constants.GUEST_ID      // placeholder mimei id
    private nonisolated let preferenceHelper: PreferenceHelper
    static var appUser = User(mid: Constants.GUEST_ID)
    ...
    func initialize() async throws {
        // Initialize app user through AppUserStore
        await AppUser.shared.initialize(preferenceHelper: preferenceHelper)
        
        await TweetCacheManager.shared.deleteExpiredTweets()
    }
    ...
    func fetchTweetFeed(
        ...
        for item in response {
            if let tweetDict = item {
                do {
                    nonisolated(unsafe) let nonisolatedDict = tweetDict
                    let tweet = try await Tweet.from(dict: nonisolatedDict)
                    let author = try await getUser(tweet.authorId)
                    await MainActor.run {
                        tweet.author = author
                    }
                    // Save tweet back to cache
                    await TweetCacheManager.shared.saveTweet(tweet, userId: Self.appUser.mid)
                    tweets.append(tweet)
                } catch {
    ...
    func fetchComments(
        ...
        for item in response {
            if let dict = item {
                do {
                    nonisolated(unsafe) let nonisolatedDict = dict
                    let comment = try await Tweet.from(dict: nonisolatedDict)
                    let author = try? await getUser(comment.authorId)
                    await MainActor.run {
        ...
        for item in response {
            if let tweetDict = item {
                do {
                    ...
                    // Save tweet back to cache
                    // await TweetCacheManager.shared.saveTweet(tweet, userId: user.mid)
                    tweets.append(tweet)
                } catch {
                    print("Error processing tweet: \(error)")
                    tweets.append(nil)
                ...
                if let origId = tweet.originalTweetId, let origAuthorId = tweet.originalAuthorId {
                    if await TweetCacheManager.shared.fetchTweet(mid: origId) == nil {
                        if let origTweet = try? await getTweet(tweetId: origId, authorId: origAuthorId) {
                            await TweetCacheManager.shared.saveTweet(origTweet, userId: origAuthorId)
                        }
                    }
                }
                return tweet
    ...
    func getUser(_ userId: String, baseUrl: String = "") async throws -> User? {
        // Step 1: Check user cache in Core Data
        if !(await TweetCacheManager.shared.shouldRefreshUser(mid: userId)) {
        ...
        if baseUrl != Self.appUser.baseUrl {
        ...
        if let userDict = response as? [String: Any] {
            nonisolated(unsafe) let nonisolatedDict = userDict
            let user = User.from(dict: nonisolatedDict)
            user.baseUrl = baseUrl
            
            // Save to Core Data cache
            await TweetCacheManager.shared.saveUser(user)
            return user
        }
    ...
    func login(_ loginUser: User) async throws -> LoginResult {
        let entry = "login"
        let params = [
            "aid": Self.appId,
            "ver": "last",
        ...
        if let status = response["status"] as? String {
            if status == "failure" {
                if let reason = response["reason"] as? String {
                    return LoginResult(status: "failure", reason: reason)
                }
                return LoginResult(status: "failure", reason: "Unknown error occurred")
            } else if status == "success" {
                preferenceHelper.setUserId(loginUser.mid)
                return LoginResult(status: "success", reason: "Success")
            }
        }
        return LoginResult(status: "failure", reason: "Invalid response status")
    }
    ...
    func getFollows(
        ...
        if user.baseUrl != Self.appUser.baseUrl {
            let newClient = HproseHttpClient()
            newClient.timeout = 60
    ...
    func getUserTweetsByType(
        ...
        for dict in response {
            if let item = dict {
                do {
                    nonisolated(unsafe) let nonisolatedDict = item
                    let tweet = try await Tweet.from(dict: nonisolatedDict)
                    if (tweet.author == nil) {
                        tweet.author = try? await getUser(tweet.authorId)
                    }
                    await TweetCacheManager.shared.saveTweet(tweet, userId: tweet.authorId)
    ...
    func toggleFavorite(_ tweet: Tweet) async throws -> Tweet? {
        ...
        let params = [
            ...
            "userhostid": Self.appUser.hostIds?.first as Any
        ]
        guard let service = hproseClient else {
            throw NSError(domain: "HproseService", code: -1, userInfo: [NSLocalizedDescriptionKey: "Service not initialized"])
        }
        ...
        if let isFavorite = response["isFavorite"] as? Bool,
           let favoriteCount = response["count"] as? Int {
            ...
            return await tweet.copy(favorites: updatedFavorites, favoriteCount: favoriteCount)
        }
    ...
    func toggleBookmark(_ tweet: Tweet) async throws -> Tweet?  {
        ...
        let appUser = await AppUser.shared
        let params = [
            "aid": Self.appId,
            "ver": "last",
            "userid": appUser.mid,
        ...
        if let hasBookmarked = response["hasBookmarked"] as? Bool,
           let bookmarkCount = response["count"] as? Int {
            ...
            let updatedFavorites = favorites
            return await tweet.copy(favorites: updatedFavorites, bookmarkCount: bookmarkCount)
        }
    ...
    struct DeleteCommentResponse: Sendable {
        let commentId: String
        let count: Int
    }
    
    func deleteComment(parentTweet: Tweet, commentId: String) async throws -> DeleteCommentResponse? {
        ...
        if let commentId = response["commentId"] as? String,
           let count = response["count"] as? Int {
            return DeleteCommentResponse(commentId: commentId, count: count)
        }
        return nil
    }
    ...
    func uploadToIPFS(
        ...
        guard let writableUrl = try await Self.appUser.resolvedWritableUrl() else {
            throw NSError(domain: "HproseService", code: -1, userInfo: [NSLocalizedDescriptionKey: "WritableUrl not initialized"])
        }
        
        let newClient = HproseHttpClient()
        newClient.timeout = 60
        newClient.uri = "\(writableUrl)/webapi/"
        let service = newClient.useService(HproseService.self) as AnyObject
...
//    }
    ...
    /// Handles background processing tasks for tweet uploads
    static func handleBackgroundTask(task: BGProcessingTask) async {
        // Schedule the next background task
        await scheduleNextBackgroundTask()
    ...
    private static func scheduleNextBackgroundTask() async {
    ...
    /// Schedules a tweet for background upload
    ...
    func uploadTweet(_ tweet: Tweet) async throws -> Tweet? {
        ...
        let params: [String: Any] = [
            "aid": Self.appId,
            "ver": "last",
            "hostid": Self.appUser.hostIds?.first as Any,
    ...
    /**
     * Return a list of pinned tweets with their pin timestamps.
     */
    func getPinnedTweets(user: User) async throws -> [PinnedTweet] {
        guard let service = hproseClient else {
            throw NSError(domain: "HproseService", code: -1, userInfo: [NSLocalizedDescriptionKey: "Service not initialized"])
        }
        ...
        var result: [PinnedTweet] = []
        for dict in response {
            if let tweetDict = dict["tweet"] as? [String: Any],
               let timePinned = dict["timestamp"] as? TimeInterval {
                nonisolated(unsafe) let nonisolatedDict = tweetDict
                let tweet = try await Tweet.from(dict: nonisolatedDict)
                tweet.author = try? await getUser(tweet.authorId)
                result.append(PinnedTweet(tweet: tweet, timePinned: timePinned))
            }
        }
        return result
    }
    ...
    func updateUserCore(
        ...
        let updatedUser = User(mid: Self.appUser.mid, name: alias, password: password, profile: profile)
        if let hostId = hostId, !hostId.isEmpty {
    ...
    func getHostIP(_ nodeId: String) async -> String? {
        let urlString = "\(Self.appUser.baseUrl ?? HproseInstance.baseUrl)/getvar?name=ips&arg0=\(nodeId)"
