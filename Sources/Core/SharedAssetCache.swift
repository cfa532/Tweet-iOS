...
class SharedAssetCache: ObservableObject {
    ...
    private func createCachingPlayer(for url: URL, tweetId: String?) async throws -> AVPlayer {
        ...
        NSLog("DEBUG: [SHARED ASSET CACHE] Creating player for HLS video: \(url.absoluteString), mediaID: \(mediaID)")
        
        // CRITICAL: Check if we have a natively downloaded asset first (Apple's AVAssetDownloadTask)
        // This provides instant playback with no HTTP redirect delays
        if let downloadedAsset = await AVAssetDownloadManager.shared.getDownloadedAsset(for: mediaID) {
            NSLog("DEBUG: [SHARED ASSET CACHE] ✅ Using natively downloaded asset for instant playback: \(mediaID)")
            let playerItem = AVPlayerItem(asset: downloadedAsset)
            let player = AVPlayer(playerItem: playerItem)
            
            // Cache the player
            await MainActor.run { cachePlayer(player, for: mediaID) }
            
            NSLog("DEBUG: [SHARED ASSET CACHE] Player created from downloaded asset for mediaID: \(mediaID)")
            return player
        }
        
        NSLog("DEBUG: [SHARED ASSET CACHE] No downloaded asset found, using CachingPlayerItem approach for mediaID: \(mediaID)")
        
        // Start background download for future instant playback
        await AVAssetDownloadManager.shared.downloadAsset(for: url, mediaID: mediaID, title: "Video \(mediaID.prefix(8))")
...
// MARK: - AVAssetDownloadManager

/// Manages downloading and persisting HLS videos using Apple's native AVAssetDownloadTask
/// Based on: https://developer.apple.com/documentation/AVFoundation/using-avfoundation-to-play-and-persist-http-live-streams
@MainActor
class AVAssetDownloadManager: NSObject, ObservableObject {
    
    static let shared = AVAssetDownloadManager()
    
    // Track download tasks
    private var activeTasks: [String: AVAggregateAssetDownloadTask] = [:]
    private var downloadedAssets: [String: AVURLAsset] = [:]
    ...
    private func setupDownloadSession() {
        let config = URLSessionConfiguration.background(withIdentifier: "com.tweet.hls-download")
        ...
        NSLog("DEBUG: [AVAssetDownloadManager] Download session configured")
    }
    
    /// Check if an asset has been downloaded
    func hasDownloadedAsset(for mediaID: String) -> Bool {
        return downloadedAssets[mediaID] != nil
    }
    
    /// Get a downloaded asset if available
    func getDownloadedAsset(for mediaID: String) -> AVURLAsset? {
        return downloadedAssets[mediaID]
    }
    
    /// Start downloading an HLS video for offline playback
    func downloadAsset(for url: URL, mediaID: String, title: String = "Video") {
        // Skip if already downloaded or downloading
        if downloadedAssets[mediaID] != nil {
            NSLog("DEBUG: [AVAssetDownloadManager] Asset already downloaded for mediaID: \(mediaID)")
            return
        }
        
        if activeTasks[mediaID] != nil {
            NSLog("DEBUG: [AVAssetDownloadManager] Download already in progress for mediaID: \(mediaID)")
            return
        }
        
        NSLog("DEBUG: [AVAssetDownloadManager] Starting background download for mediaID: \(mediaID)")
        
        // Create AVURLAsset
        let asset = AVURLAsset(url: url)
        
        // Start aggregate download task
        let task = downloadSession.aggregateAssetDownloadTask(
            with: asset,
            mediaSelections: [asset.preferredMediaSelection],
            assetTitle: title,
            assetArtworkData: nil,
            options: [
                AVAssetDownloadTaskMinimumRequiredMediaBitrateKey: 265000
            ]
        )
        ...
        NSLog("DEBUG: [AVAssetDownloadManager] Background download started for mediaID: \(mediaID)")
    }
    
    /// Cancel download
    func cancelDownload(for mediaID: String) {
        guard let task = activeTasks[mediaID] else { return }
        task.cancel()
    ...
    func getProgress(for mediaID: String) -> Double {
        return downloadProgress[mediaID] ?? 0.0
    }
}
...
extension AVAssetDownloadManager: AVAssetDownloadDelegate {
    ...
    nonisolated func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        Task { @MainActor in
            ...
            // Find mediaID
            var foundMediaID: String?
            for (mediaID, activeTask) in activeTasks {
                if activeTask == downloadTask {
                    foundMediaID = mediaID
                    break
                }
            }
            
            guard let mediaID = foundMediaID else { return }
            
            if let error = error {
                NSLog("DEBUG: [AVAssetDownloadManager] Download failed for \(mediaID): \(error.localizedDescription)")
                activeTasks.removeValue(forKey: mediaID)
                downloadProgress.removeValue(forKey: mediaID)
                return
            }
            ...
            // Success!
            NSLog("DEBUG: [AVAssetDownloadManager] ✅ Download completed for \(mediaID)")
            
            let urlAsset = downloadTask.urlAsset
            downloadedAssets[mediaID] = urlAsset
            NSLog("DEBUG: [AVAssetDownloadManager] Stored downloaded asset, location: \(urlAsset.url.absoluteString)")
    ...
    nonisolated func urlSession(_ session: URLSession, aggregateAssetDownloadTask: AVAggregateAssetDownloadTask, didLoad timeRange: CMTimeRange, totalTimeRangesLoaded loadedTimeRanges: [NSValue], timeRangeExpectedToLoad: CMTimeRange, for mediaSelection: AVMediaSelection) {
        
        Task { @MainActor in
            // Find mediaID
            var foundMediaID: String?
            for (mediaID, activeTask) in activeTasks {
                if activeTask == aggregateAssetDownloadTask {
                    foundMediaID = mediaID
                    break
                }
            }
            ...
            for value in loadedTimeRanges {
                ...
                totalDuration += range.duration.seconds
            }
            
            let expectedDuration = timeRangeExpectedToLoad.duration.seconds
            let progress = expectedDuration > 0 ? min(totalDuration / expectedDuration, 1.0) : 0.0
