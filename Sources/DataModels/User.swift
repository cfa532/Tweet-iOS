...
// Global AppUser object
@MainActor
final class AppUser: User, @unchecked Sendable {
    static let shared = AppUser()
    
    private override init(mid: String = Constants.GUEST_ID,
                         baseUrl: String? = nil,
                         name: String? = nil,
                         username: String? = nil,
                         password: String? = nil,
    ...
    func initialize(preferenceHelper: PreferenceHelper) async {
        ...
        self.followingList = Gadget.getAlphaIds()
        ...
        for url in preferenceHelper.getAppUrls() {
            do {
                ...
                if let firstIp = Gadget.shared.filterIpAddresses(addrs) {
                    ...
                    if !isGuest,
                       let user = try await HproseInstance.shared.getUser(mid, baseUrl: baseUrl) {
                        ...
                        self.baseUrl = HproseInstance.baseUrl
                        self.followingList = followings
                        return
                    } else {
                        self.mid = Constants.GUEST_ID
                        self.baseUrl = HproseInstance.baseUrl
                        self.followingList = Gadget.getAlphaIds()
                        return
                    }
    ...
    func updateFrom(_ newUser: User) {
        self.mid = newUser.mid
        self.baseUrl = newUser.baseUrl
        self.writableUrl = newUser.writableUrl
        self.name = newUser.name
        self.username = newUser.username
        self.avatar = newUser.avatar
        self.email = newUser.email
        self.profile = newUser.profile
        self.cloudDrivePort = newUser.cloudDrivePort
        
        self.tweetCount = newUser.tweetCount
        self.followingCount = newUser.followingCount
        self.followersCount = newUser.followersCount
        self.bookmarksCount = newUser.bookmarksCount
        self.favoritesCount = newUser.favoritesCount
        self.commentsCount = newUser.commentsCount
        
        self.hostIds = newUser.hostIds
    }
}
...
class User: ObservableObject, Codable, Identifiable, Hashable, @unchecked Sendable {
    ...
    static func from(dict: [String: Any]) -> User {
        do {
            ...
            return User(mid: Constants.GUEST_ID)
        }
    }
    ...
    static func from(cdUser: CDUser) -> User {
        ...
        return User(mid: cdUser.mid ?? Constants.GUEST_ID)
    }
    
    func update(_ from: User) {
        // update self with the user input. Do not update baseUrl and writableUrl
        self.mid = from.mid
        self.name = from.name
        ...
        self.lastLogin = from.lastLogin
        self.cloudDrivePort = from.cloudDrivePort
        
        self.tweetCount = from.tweetCount
        self.followingCount = from.followingCount
        self.followersCount = from.followersCount
        self.bookmarksCount = from.bookmarksCount
        self.favoritesCount = from.favoritesCount
        self.commentsCount = from.commentsCount
        
        self.fansList = from.fansList
        self.followingList = from.followingList
    }
