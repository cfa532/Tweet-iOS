...
# FFmpeg iOS Framework Build Script for HLS Video Processing
# Optimized for video to HLS conversion only

# directories
SCRATCH=`pwd`/"scratch"
ARCHS="arm64 x86_64"  # Simplified for modern iOS devices and simulator

FFMPEG_VERSION="7.1"  # Updated to latest stable version
...
HLS_ENABLED_CODECS="h264 aac mp3"
HLS_ENABLED_FORMATS="mp4 m3u8 ts"
...
function CompileSource() {
  # Build FFmpeg with HLS-specific configuration
  ./build-ffmpeg.sh $ARCHS --enable-gpl --enable-libx264 --enable-libfdk-aac --enable-nonfree
...
function RenameHeader() {
  ...
  # Only copy headers needed for HLS processing
  local hls_headers=("avcodec" "avformat" "avutil" "swscale" "swresample")
  
  for folder_name in "${hls_headers[@]}"; do
    local folder="$include_folder/$folder_name"
    if [ -d "$folder" ]; then
  ...
  # Remove include prefixes for cleaner imports
...
function CreateModulemapAndUmbrellaHeader() {
  # Create ffmpeg.h with only HLS-relevant headers
  cat > $OUTPUT_UMBRELLA_HEADER <<EOF
#import <Foundation/Foundation.h>
#import <VideoToolbox/VideoToolbox.h>
#import <AudioToolbox/AudioToolbox.h>

// Core FFmpeg libraries for HLS processing
#include "avcodec.h"
#include "avformat.h"
#include "avutil.h"
#include "swscale.h"
...
#define FFMPEG_HLS_SEGMENT_DURATION 6.0
#define FFMPEG_HLS_TARGET_BITRATE 1000000
#define FFMPEG_HLS_AUDIO_BITRATE 128000

// Version information
double FFmpegVersionNumber = $FFMPEG_VERSION;

// HLS processing function declarations
int ffmpeg_convert_to_hls(const char* input_path, const char* output_dir, 
                         int width, int height, int bitrate, int audio_bitrate);
int ffmpeg_create_hls_playlist(const char* output_dir, const char* playlist_name);
EOF
...
framework module $FRAMEWORK_NAME {
  ...
  // Link against required system frameworks for HLS
...
<dict>
        ...
        <string>FFmpeg framework optimized for HLS video processing</string>
</dict>
</plist>
EOF
}
...
# Main execution
echo "Building FFmpeg iOS Framework for HLS Video Processing..."
echo "Version: $FFMPEG_VERSION"
echo "Architectures: $ARCHS"
echo "Enabled codecs: $HLS_ENABLED_CODECS"
echo "Enabled formats: $HLS_ENABLED_FORMATS"
...
echo "FFmpeg iOS Framework build completed!"
echo "Framework location: $OUTPUT_FOLDER"
echo ""
echo "Next steps:"
echo "1. Add the framework to your Xcode project"
echo "2. Link against VideoToolbox, AudioToolbox, CoreMedia, and CoreVideo frameworks"
echo "3. Add the following to Other Linker Flags:"
echo "   -lavformat -lavcodec -lavutil -lswresample -lswscale -lz -lm -lpthread"
echo "4. Set Enable Bitcode to No"
echo "5. Use the FFmpegWrapper.swift for HLS conversion"
